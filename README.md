...
The simulator calls the existing `POST /api/traffic/analyze` inference endpoint and visualizes the returned Layer 1 → Layer 2A → Layer 2B → threat-scoring path. Presets include normal traffic, SQLi, XSS, encoded XSS, LFI, command injection, and a custom request. The displayed score, confidence, anomaly score, and latency are live values from the running model; paper evaluation metrics are not generated by the simulator.

For a final-project demo, start `dummy_app.py`, MongoDB, and the WAF, then open the simulator page and run the presets one at a time.
